...
def crm_test(out_file: str, alpha: float = 1.0, gamma: float = 1.0, epsilon: float = 0.1, name: str =''):
    with open(out_file, "w") as csvfile:
        print("crm_{}, {}: begin experiment".format(name, map_fn))
        report = SequenceReport(csvfile, LOG_STEP, init, EPISODE_LENGTH, TRIALS)
...
if __name__=="__main__":
    for i in range(0, 10):
        map_fn = './src/tests/craft/bridge-maps/map_{}.map'.format(i)

        GOAL = [OBJECTS["bridge"]]

        qrm_test("./plots/bridge-results/mprm-bridge{}.csv".format(i), alpha=0.95, name='bridge_gold_MPRM')
        qrm_test("./plots/bridge-results/seq0-bridge{}.csv".format(i), alpha=0.95, name='SEQ0')
        qrm_test("./plots/bridge-results/seq1-bridge{}.csv".format(i), alpha=0.95, name='SEQ1')
        qrm_test("./plots/bridge-results/seq2-bridge{}.csv".format(i), alpha=0.95, name='SEQ2')
        qrm_test("./plots/bridge-results/seq3-bridge{}.csv".format(i), alpha=0.95, name='SEQ3')
        qrm_test("./plots/bridge-results/pop0-bridge{}.csv".format(i), alpha=0.95, name='POP0')
        qrm_test("./plots/bridge-results/pop1-bridge{}.csv".format(i), alpha=0.95, name='POP1')

        #crm_test("./plots/bridge-results/crm-bridge{}.csv".format(i), name='bridge_gold_MPRM')